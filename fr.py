...
model_emb.load_state_dict(torch.load("model_arch/backbones/backbone.pth", map_location = device))
model_emb.to(device)
model_emb.eval()
...
def align_face(input_image, bbox, landmarks):
    ...
    """
    reference_landmarks = [[30.2946, 51.6963], [65.5318, 51.5014],
    ...
    face_img = cv2.warpAffine(input_image, tform.params[0:2, :], (112, 112), borderValue=0.0)
...
def main():
    ...
    while(True):
        ...
        # landmarks color
        colors = [(0, 255, 255), (0, 255, 255), (0, 255, 255), (0, 255, 255), (0, 255, 255)]    
        # Get boxs
        for i in range(len(bboxs)):
            # Get location face
            x1, y1, x2, y2 = bboxs[i]
